...
def create_app(config_path="config.yml", secrets_path="secrets.yml"):
    ...
    async def run_refresh_models():
        while True:
            await app.state.composer.refresh_models()
            await asyncio.sleep(1)

    async def run_refresh_metrics():
        while True:
            await app.state.composer.refresh_metrics()
    ...
    async def health_check():
        ...
        health_data = []
        for server in app.state.composer.servers:
            server_url = server["url"]
            health_data.append({
                "url": server_url,
                "healthy": app.state.composer.server_health.get(server_url, {}).get("healthy", True),
                "metrics_cached": app.state.composer.metrics_cache.get(server_url),
                "model_cached": app.state.composer.model_cache.get(server_url)
            })
    ...
    async def get_aggregated_metrics():
        ...
        """
        metrics_data = {}
        app.state.composer.logger.info("Fetching metrics from all servers...")
        async with httpx.AsyncClient() as client:
            ...
            for server, result in zip(app.state.composer.servers, results):
                if isinstance(result, Exception):
                    app.state.composer.logger.error(f"Error fetching metrics from {server['url']}: {result}")
    ...
    async def reload_configuration(request: Request):
        ...
        user_group = app.state.composer.get_group_for_token(user_token)
        if not user_group:
        ...
        if user_group not in app.state.composer.admin_groups:
            raise HTTPException(status_code=403, detail="Forbidden: Insufficient permissions")
        
        app.state.composer.logger.info(f"User '{user_group}' authorized to reload configuration.")

        # Reload configuration and secrets
        try:
            app.state.composer.load_config()
            app.state.composer.load_secrets()
            app.state.composer.logger.info("Configuration and secrets reloaded successfully.")
            return JSONResponse(content={"message": "Configuration and secrets reloaded successfully."})
        except Exception as exc:
            app.state.composer.logger.error(f"Error reloading configuration or secrets: {exc}")
    ...
    async def proxy_request(path: str, request: Request):
        ...
        if not user_group:
            raise HTTPException(status_code=403, detail="Forbidden: Invalid token or unauthorized group")
        app.state.composer.logger.info(f"User '{user_group}' authenticated.")
        ...
        if path == "models":
            app.state.composer.logger.info("Received request for aggregated models from all servers.")
            return await app.state.composer.handle_models_request(user_group)
        ...
        app.state.composer.logger.info(f"Received request for model '{target_model_name}' from group '{user_group}'.")

        # Find compatible servers for the target model and user group
        compatible_servers = await app.state.composer.get_compatible_servers(target_model_name, user_group)
        ...
        least_loaded_server = await app.state.composer.get_least_utilized_server(compatible_servers)
        ...
        app.state.composer.logger.info(f"Least loaded server for model '{target_model_name}': {least_loaded_server}.")
        app.state.composer.logger.debug(f" Forwarding request with payload: {payload}")
        ...
        app.state.composer.logger.info(f"Forwarding request to {url} with headers: {headers}")
        headers["Authorization"] = f"Bearer {app.state.composer.vllm_token}"

        # Register the time of utilization in app.state.composer.servers
        server_idx = [i for i, server in enumerate(app.state.composer.servers) if server["url"] == least_loaded_server][0]
        app.state.composer.servers[server_idx]["last_utilization"] = datetime.utcnow()

        # Forward the request either in streaming mode or non-streaming mode
        stream_mode = payload.get("stream", False)
        app.state.composer.logger.info(f"Streaming mode: {stream_mode}")
        if stream_mode:
            ...
            try:
                ...
                async def safe_stream_generator():
                    try:
                        ...
                        app.state.composer.logger.error(f"Error during streaming: {exc}")
            ...
            try:
                ...
                app.state.composer.logger.debug(f"Backend response headers: {dict(response.headers)}")
                app.state.composer.logger.debug(f"Backend response content: {response.content}")
                return Response(
