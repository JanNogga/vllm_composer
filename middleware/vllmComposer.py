...
class vllmComposer:
    ...
    async def handle_models_request(self, user_group: str) -> JSONResponse:
        ...
        for server in self.servers:
            ...
            if server_models:
                self.logger.debug(f"Found model {server_models} on server {server_url}. Type of server_models: {type(server_models)}")
                models_data.append(server_models)

        # Deduplicate and format the data as expected by the client
        self.logger.debug(f"Proceeding to format {models_data} for response.")
